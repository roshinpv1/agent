...
from utils.parse_utils import parse_llm_response, get_structured_relationships, convert_to_index
...
```"""
        ...
        # --- Validation ---
        try:
            parsed_data, format_type = parse_llm_response(response)
            
            if not isinstance(parsed_data, list):
                raise ValueError("LLM Output is not a list")
                
            abstractions = parsed_data  # Directly use the parsed data
            ...
            for item in abstractions:
                ...
                for idx_entry in item["file_indices"]:
                    try:
                        idx = convert_to_index(idx_entry)
            ...
            print(f"Error processing abstractions: {e}")
            print(f"Raw response: {response}")
            # Provide a basic fallback abstraction if everything fails
            return [
                {
                    "name": "Core System",
                    "description": "The main functionality of the system. We couldn't parse the LLM response properly.",
                    "files": [0] if file_count > 0 else []
                }
            ]
...
"""
        ...
        try:
            ...
            # Get structured relationships with proper validations
            relationships_data = get_structured_relationships(parsed_data, num_abstractions)
            ...
            print(f"Raw response: {response}")
            # Generate fallback relationships if everything fails
            fallback_rels = []
            for i in range(min(num_abstractions-1, 2)):  # Create up to 2 relationships
            ...
            return {
                "summary": f"Project with {num_abstractions} components",
                "details": fallback_rels
            }
...
"""
        ...
        try:
            ...
            ordered_indices_raw = parsed_data

            ordered_indices = []
            seen_indices = set()
            for entry in ordered_indices_raw:
                try:
                    idx = convert_to_index(entry)

                    if not (0 <= idx < num_abstractions):
                        print(
                            f"Warning: Invalid index {idx} in ordered list. Max index is {num_abstractions-1}. Skipping."
                        )
                        continue
                        
                    if idx in seen_indices:
                        print(f"Warning: Duplicate index {idx} found in ordered list. Skipping.")
                        continue
                    ...
                    print(
                        f"Warning: Could not parse index from ordered list entry: {entry}. Skipping."
                    )

            # Check if all abstractions are included and add missing ones
            if len(ordered_indices) != num_abstractions:
                missing_indices = set(range(num_abstractions)) - seen_indices
                print(f"Warning: Missing indices in ordered list: {missing_indices}. Adding them at the end.")
                ordered_indices.extend(sorted(list(missing_indices)))

            print(f"Determined chapter order (indices): {ordered_indices}")
            return ordered_indices  # Return the list of indices
            
        except Exception as e:
            print(f"Error processing chapter order: {e}")
            print(f"Raw response: {response}")
            # Generate a simple sequential order as fallback
            return list(range(num_abstractions))
