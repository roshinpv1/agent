...
LOCAL_MODEL_DIR = "/Users/roshinpv/.cache/torch/sentence_transformers" 


def get_application_analysis( query : str):

    """
    Answer any query related to the application's behavior, logs, alerts, error handling, performance, resilience patterns, and Non-Functional Requirements (NFRs) .
    """
...
root_agent = Agent(
    ...
    name='cloud_gates',
    instruction = '''You are a helpful agent that answers any query related to the application's behavior, logs, alerts, error handling, performance, resilience patterns, and Non-Functional Requirements (NFRs). Use only the `get_application_analysis` tool for all application-related and NFR-related queries. Base your responses strictly on the data derived from static analysis of the source code.''',
    description = '''This agent specializes in retrieving detailed analysis of the application's implementation, focusing on logging practices, error handling, availability, auditability, and other Non-Functional Requirements (NFRs). It also evaluates the application's cloud readiness and architectural resilience. Use only the `get_application_analysis` tool for all application and NFR-related queries.''',
    tools=[get_application_analysis], # You can add Python functions directly to the tools list; they will be automatically wrapped as FunctionTools.
)
...
call_agent("Get me the application analysis report for BPAY")
